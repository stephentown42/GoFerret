function level25_InfoMaskTargetted
try
...
%__________________________________________________________________________    
    case('PrepareStim')
        ...
        if ~isfield(gf,'nStim'), 
            ...
            gf.maskCases = [-1 0 randperm(numel(gf.maskLag))];            
        ...
        % Determine sound duration based on mask
        gf.duration = 250;      gf.reportMask = NaN;
...
%         if ismember(gf.formants,[730 2058 2857 4205],'rows'), gf.atten = gf.atten - 2; end
        ...
        % Generate distractor vowel         
        eval( sprintf('nForm = gf.sound%d;', ~gf.side));
        
        noise = newMakeVowel2009( 0.1, gf.fStim, gf.pitch, nForm(1),nForm(2),nForm(3), nForm(4));
        noise = noise .* 10^(-(gf.atten/20));
        noise = envelope(noise,ceil(0.005.*gf.fStim));
        
        % Calibrate distractor vowel        
        noise0 = conv(noise, gf.fltL.flt, 'same');
        noise1 = conv(noise, gf.fltR.flt, 'same');
        
        [Left_attn, Right_attn] = getCalibAtten(65.5,nForm);
        noise0 = noise0 .* 10 ^(-(Left_attn/20));
        noise1 = noise1 .* 10 ^(-(Right_attn/20));

        % Assign signal and noise to speakers depending on mask case   
        % (Do nothing if level zero)
        if gf.mask > 0,
            
                      lag = gf.maskLag( gf.mask);               % Lag in seconds
            gf.reportMask = lag;                                % Capture for output file
                      lag = max([1 ceil(lag.*gf.fStim)]);       % Convert to samples 
                      
            
            % Add to sound
            sound0(lag:lag+numel(noise0)) = sound0(lag:lag+numel(noise0)) + [noise0 0];
            sound1(lag:lag+numel(noise1)) = sound1(lag:lag+numel(noise1)) + [noise1 0];
        end  
...
% Center Response__________________________________________________________        
    case('WaitForStart')
        ...
        if centerLick == 0;
            ...
            comment = sprintf('LED flashing, waiting for center lick: noise lag = %d ms', gf.reportMask*1000);

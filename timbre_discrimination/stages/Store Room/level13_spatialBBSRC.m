...
%         if ismember(gf.formants,[460 1105 2857 4205],'rows'), gf.atten = gf.atten - 5; end        
        ...
        sound  = [sound, isi, sound];  

        % Generate distractor vowel                        
        distractor = newMakeVowel2009(  gf.duration/1000,...
                                        gf.fStim, ...
                                        gf.pitch,...
                                        gf.Dvow(1),...
                                        gf.Dvow(2),...
        ...
        distractor = envelope(distractor,250);
        distractor = distractor.*10^(-((gf.atten)/20));
        distractor = [distractor, isi, distractor];  

        % Organize space    
        switch gf.mask,

            case 0              % 
                gf.comment = 'Both from left';
                sound0 = distractor + sound;
                sound1 = 0.*sound;

            case 1              % 
                gf.comment = 'Both from  right';
                sound0 = 0.* sound;
                sound1 = distractor + sound;

            case 2              % 
                gf.comment = 'Signal left; noise right';
                sound0 = sound;
                sound1 = distractor;

            case 3              % 
                gf.comment = 'Signal right; noise left';
                sound0 = distractor;
                sound1 = sound;

            case 4              % 
                gf.comment = 'Signal left only';
                sound0 = sound;
                sound1 = distractor.*0;
            case 5              % 
                gf.comment = 'Signal right only';
                sound0 = distractor.*0;
                sound1 = sound;
        end
        ...
        set(h.comment, 'string', gf.comment);
...
% Center Response__________________________________________________________        
    case('WaitForStart')
        ...
        if centerLick == 0;
            ...
            comment = strcat(gf.comment, '     LED flashing, waiting for center lick');

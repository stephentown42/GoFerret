...
switch gf.status
    ...
    case('PrepareStim')
      ...
      %  gf.atten = 18;         % temporary compensation for new speakers until something better arrrives
        ...
        startRef1=0.5*gf.fStim; % start point (in the middle of the noise)
...
%         %Check peripheral sensor inputs
% tarIdx    = [2,8];
% checkPeri = [0,0]; 
% 
% for ii = 1 : 2
%     checkPeri(ii) = DA.GetTargetVal( sprintf( '%s.lick%d',  gf.stimDevice, tarIdx(ii)));
% end
% 
% centerDisable = any(checkPeri);
% 
% if centerDisable,
%     gf.lastPeripheralResp = now; 
%     % Disable center spout
%     DA.SetTargetVal( sprintf('%s.centerEnable', gf.stimDevice), 0);
% else
%     timeSinceLastPeriphResp = now - gf.lastPeripheralResp;              % Serial date format
%     timeSinceLastPeriphResp = (24*60*60) * timeSinceLastPeriphResp;     % Convert to seconds
% 
%     centerDisable = timeSinceLastPeriphResp < 0.6;
%     
%     % Disable center spout
%     DA.SetTargetVal( sprintf('%s.centerEnable', gf.stimDevice), 0);
% end

...
switch gf.status
    ...
    case('PrepareStim')
        ...
        % Decide whether it should be a double sound presentation
        d=rand(1);
        fused=0;
        gf.distractorLoc= 99;
        if d<=0.5
            gf.distractorLoc= gf.refChoice;
            fused=1;
            index=[3,10,4,11,5,12,6,13,7];
            f1=index(find(index==gf.refChoice)-1);
            f2=index(find(index==gf.refChoice)+1);
        end
        ...
        if fused==0
            ...
            stimF1 = WEcalibration(stimSound(1,:), gf.calibDir, f1);
            stimF2 = WEcalibration(stimSound(1,:), gf.calibDir, f2);
        end
        ...
        if fused==0
            ...
            fref(1,:)=stimF1(1,startRef1:startRef1+refD-1); % get the samples
            fref(2,:)=stimF2(1,startRef1:startRef1+refD-1); % get the samples
        end
        ...
        if fused==0
            ...
            for ii=1:size(fref,1) % envelope the samples
                fref(ii,:)=envelopeKW(fref(ii,:),5,gf.fStim);
            end
        end
        ...
        if fused==0
            ...
            for ff=1:2
                for ii=1:gf.nRefSpks % create reference sound
                    frsound(ff,:)=[(zeros(1,floor(gf.refDuration*gf.fStim*(ii-1)))),...
                        zeros(1,floor(gf.interval*gf.fStim*(ii-1))),...
                        fref(ff,:),...
                        zeros(1,floor(gf.interval*gf.fStim)),...
                        (zeros(1,floor(gf.refDuration*gf.fStim*(gf.nRefSpks-ii)))),...
        ...
        if fused==0
            ...
            gf.silence=zeros(1,size(frsound,2));% Fill unused speakers with silence
        end
        ...
        if fused==0
            ...
            if gf.refChoice==4 | gf.refChoice==5 | gf.refChoice==6 
                for ii=1:2
                    frsound(ii,:) = frsound(ii,:) .* 10^(-((gf.atten+6)/20));
                end
            elseif gf.refChoice==10 | gf.refChoice==11 
                  for ii=1:2
                    frsound(ii,:) = frsound(ii,:) .* 10^(-((gf.atten+4.5)/20));
                end
            else
                for ii=1:2
                    frsound(ii,:) = frsound(ii,:) .* 10^(-((gf.atten+1.5)/20));
                end
            end
        end
        ...
        uSp=[3,4,5,6,7,10,11,12,13];
        for ii=1:length(uSp)
            silentSpks=strcat('%s.speaker',num2str(uSp(ii)));
        ...
        if fused==0
            ...
            refractS  = playDelay + length(frsound) + ceil(gf.refractoryTime * gf.fStim);
        end
        ...
        if fused==0
            ...
            refSpeaker{1} = strcat('%s.speaker',num2str(f1));
            refSpeaker{2} = strcat('%s.speaker',num2str(f2));
        end
        ...
        if fused==0
            ...
            DA.WriteTargetVEX(sprintf(refSpeaker{1}, gf.stimDevice), 0, 'F32', frsound(1,:)); % Write to reference speaker
            DA.WriteTargetVEX(sprintf(refSpeaker{2}, gf.stimDevice), 0, 'F32', frsound(2,:)); % Write to reference speaker
        end
    ...
    case('WaitForResponse')
        ...
        if ~sum(pResp)
            ...
            if corrResp
                ...
                if gf.correctionTrial==0 && response==1 % to try to counter left bias, reward extra at right on non-correction trials
                    valve_WE(gf.side,gf.rewardTime+40,1)
                elseif gf.correctionTrial==0 && response==0 % to try to counter left bias, reward less at left on non-correction trials
                    valve_WE(gf.side,gf.rewardTime,1)
                else
                    valve_WE(gf.side,gf.rewardTime,1) % normal on correction trials
                end

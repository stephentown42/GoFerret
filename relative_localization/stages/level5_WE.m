...
%           gf.atten = 18;         % temporary compensation for new speakers until something better arrrives
        ...
        startRef1=0.5*floor(gf.fStim); % start point (in the middle of the noise)
        ...
        holdOK      = ((gf.refDuration*gf.nRefSpks)+(gf.tarDuration/2)+gf.interval)*gf.fStim; % point in stimulus that the ferret must hold to
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%% NB interval added in 30/09/2014
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%         end
    ...
    case('WaitForResponse')
        ...
        if gf.singPres==1 && gf.correctionTrial==0 && gf.nSingPres>=gf.nCT
        ...
        if ~sum(pResp)
            ...
            if corrResp
                ...
                if gf.side==2 & gf.tarChoice~=gf.refChoice
                    valve_WE(gf.side,gf.rewardTime,1)
                elseif gf.side==8 & gf.tarChoice~=gf.refChoice
                    valve_WE(gf.side,gf.rewardTime,1)
                elseif gf.tarChoice==gf.refChoice
                    if rand(1)<=0.7
                    valve_WE(gf.side,gf.rewardTime,1)
                    end
                end
    ...
    case('timeout')
        ...
        if gf.timeoutPlayed==0
            ...
            gf.toneTime=DA.GetTargetVal(sprintf('%s.TOtime',gf.stimDevice)) ./ gf.fStim;  %Open Ex

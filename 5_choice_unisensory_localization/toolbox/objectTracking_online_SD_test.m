function objectTracking_online_SD_test
...
blobs = regionprops(BW,'Area','ConvexHull');
...
blobSize = cell2mat(blobs(1,:));
biggestBlob = blobSize == max(blobSize);
blobBoundary = blobs{2, biggestBlob};
...
%             cam.ferretIntensity = fread(cam.tcpip);
%         else
%             cam.ferretIntensity = 0.6
%         end
...
%         if timeElapsed > cam.timeLimit || GFoff
         if cam.GFexit  

...
%__________________________________________________________________________    
    case('PrepareStim')%none to prepare
        ...
        if ~isfield(gf,'seed')
            ...
            gf.seed.next    = 2; % Queue audiovisual trial
            gf.correctCount = 0;
        ...
        if gf.correctCount > gf.correctThreshold(gf.modality+1)                  
           ...
           gf.correctCount  = 0;           
           if gf.seed.current == 2
               gf.seed.next = 1 - gf.seed.previous;      % Queue opposite modality
           else
               gf.seed.next = 2;
           end
        ...
        if gf.modality ==2
            gf.domMod = gf.seed.previous;
        end
                
        % Generate stimulus based on modality
        switch gf.modality            
            case 0
                gf.stimGrid = generateStimulusGrid_Level6_flipped(0,0);
            case 1
                gf.stimGrid = generateStimulusGrid_Level6_flipped(1,1);
            case 2
                gf.stimGrid = generateStimulusGrid_Level6_flipped(2, gf.seed.previous);
        end
...
% Peripheral Response______________________________________________________        
    ...
    %                 end
                ...
                % Update performance                      
                try
                    gf.performance( valveID, gf.targetSpout, gf.domMod+1) = gf.performance( valveID, gf.targetSpout, gf.domMod+1) +1;

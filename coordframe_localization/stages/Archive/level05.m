...
%__________________________________________________________________________    
    case('PrepareStim')%none to prepare
        ...
        if gf.stimIdx > numel(gf.stimOrder)
           gf = rmfield(gf,'stimGrid'); return
        end
        
        % Get current position within stimulus grid
        ...
        % Draw stimulus properties from distribution
        nSRi         = gf.modality+1;
        nStimRepeats = gf.nStimRepeats_std(nSRi) * randn(1);        % Get number from distribution
        nStimRepeats = nStimRepeats + gf.nStimRepeats_mean(nSRi);   % Add mean
        gf.nStimRepeats = round(nStimRepeats);                         % Discretize
        gf.nStimRepeats = max([1 gf.nStimRepeats]);                 % Make sure always >=1

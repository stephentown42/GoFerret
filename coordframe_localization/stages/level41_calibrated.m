function level41_calibrated

% Fixed number of trials defined by number of speakers 
% Presents A,V and AV in completely random order
% Only colocated AV trials
...
%__________________________________________________________________________    
    case('PrepareStim')%none to prepare
        ...
        if gf.correctionTrial == 0
            [gf.stim, gf.stimGrid] = getStimGrid_allowColocation(gf.stim);
        end

        gf.modality    = 2;%gf.stimGrid(1);    % 0 = LED, 1 = Speaker, 2 = Both 
        gf.domMod      = 1;%gf.stimGrid(2);
        gf.LED         = gf.stimGrid(3);
        gf.Speaker     = gf.stimGrid(3); 
        gf.targetSpout = gf.stimGrid(3);
...
% Center Response__________________________________________________________        
    case('WaitForStart')
        ...
        %If start
        if gf.centerStatus && centerLick == 1;            

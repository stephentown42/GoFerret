...
center = DA.GetTargetVal( sprintf( '%s.sensor1', gf.recDevice))*0.25 + 1.5;
left   = DA.GetTargetVal( sprintf( '%s.sensor8', gf.recDevice))*0.25 + 0.5;
right  = DA.GetTargetVal( sprintf( '%s.sensor2', gf.recDevice))*0.25 + 2.5;
...
center = DA.GetTargetVal( sprintf( '%s.lick1', gf.recDevice));
left   = DA.GetTargetVal( sprintf( '%s.lick8', gf.recDevice));
right  = DA.GetTargetVal( sprintf( '%s.lick2', gf.recDevice));
...
% for i = 4 : 6,
%    
%     y = DA.GetTargetVal( sprintf( '%s.lick%d', gf.recDevice, i));
%     
%     if y > 0,
%        
%         y = y + offset(i-3);
...
for i = 1 : 3,
    ...
    y = DA.GetTargetVal( sprintf( '%s.valve%dout',gf.recDevice,valve(i)));

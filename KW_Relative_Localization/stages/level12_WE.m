...
%           gf.atten = 18;         % temporary compensation for new speakers until something better arrrives
        ...
        if gf.rlTrialNo>size(gf.rlp,1)
         ...
         % Generate sound
        stimSound=rand(gf.nRefSpks+1,floor(gf.fStim)*4);
        ...
        refSamps(1,:)=stimSound(1,startRef1:startRef1+totalD-1); % get the samples
        refSamps(2,:)=stimSound(2,startRef2:startRef2+totalD-1); % get the samples
        tarSamps=stimSound(gf.nRefSpks+1,startTar:startTar+totalD-1); % get the samples
        
        %%%%%%%%%%%%%%%%%%%%%% SORAYA - this is the smooth sound bit
        %%%%%%%%%%%%%%%%%%%%%% %%%%%%%%%%%%%%%%
        % Now ramp the samples
        ramp=linspace(0,(pi/2),length(tarSamps));
        if gf.side==2
            tarSamps=refSamps(2,:).*sin(ramp);
        else
            tarSamps=refSamps(2,:).*sin(ramp);
        end
        for ii=1:size(refSamps,1) % envelope the samples
            if gf.side==2
                refSamps(ii,:)=refSamps(ii,:).*cos(ramp);
            else
                refSamps(ii,:)=refSamps(ii,:).*cos(ramp);
            end
        ...
        % create the stimulus
        gapp=1-(gf.refDuration*gf.nRefSpks)-gf.tarDuration;    % interval between repeated stimuli made so total stim is 1s
        ...
        holdOK      = ((gf.refDuration*gf.nRefSpks/2)*gf.fStim);                % point in stimulus that the ferret must hold to
...
%         % Calibrate for white elephant
%         for ii=1:gf.nRefSpks
%             refSound(ii,:) = WEcalibration(refSound(ii,:), gf.calibDir, gf.refChoice(ii));
%         end
%          tarSound(ii,:) = WEcalibration(tarSound(ii,:), gf.calibDir, gf.tarChoice);
%         

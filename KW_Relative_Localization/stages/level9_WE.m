    ...
    case('PrepareStim')
        ...
        if gf.filter==4
         ...
         switch gf.filter
             case 1
                 for ii=1:size(stimSound,1)
                     stimSound(ii,:)=filter(gf.lowPass.LowPassFerret,stimSound(ii,:));
                 end
             case 2
                 for ii=1:size(stimSound,1)
                     stimSound(ii,:) = filter(gf.bandPass.bandPassFerret3_7,stimSound(ii,:));
                 end
             case 3
                 for ii=1:size(stimSound,1)
                     stimSound(ii,:) = filter(gf.bandPass.sixthOctave15kHz_bp,stimSound(ii,:));
                 end
             case 4
                 for ii=1:size(stimSound,1)
                     stimSound(ii,:) = filter(gf.highPass.highPass2k,stimSound(ii,:));
                 end
         end
        ...
        if ~sum(pResp)
            ...
            if corrResp
                ...
                if response==1 & gf.correctionTrial==0
                    valve_WE(gf.side,gf.rewardTime+50,1)
                else                

...
%__________________________________________________________________________    
    case('PrepareStim')%none to prepare
        ...
        % Initialize stimulus grid
        if ~isfield(gf,'stimGrid')  
            gf.stimIdx  = 0;
            gf.stimGrid = generateStimulusGrid;
        end
        
        % Reset stimulus grid 
        if gf.stimIdx > size(gf.stimGrid,1)
           gf.stimIdx = 0;
        end
        ...
        % Get speaker
        gf.LED     = gf.stimGrid(gf.stimIdx,3);
        gf.Speaker = gf.stimGrid(gf.stimIdx,4);
        gf.LEDmux  = getMUXfamily( gf.LED); 
        gf.SPKmux  = getMUXfamily( gf.Speaker); 
          
        % Define modality  
        gf.modality = gf.stimGrid(gf.stimIdx,1);    % 0 = LED, 1 = Speaker, 2 = Both    
        
        % Define target spout based on target modality
        gf.targetSpout = gf.stimGrid(gf.stimIdx,2);
        gf.targetSpout = gf.stimGrid(gf.stimIdx,3+gf.targetSpout);
...
% Peripheral Response______________________________________________________        
    ...
    %                 end
                ...
                set(h.performanceM(gf.stimGrid(gf.stimIdx,2)+1),'CData',gf.performance(:,:,gf.modality+1))

...
%__________________________________________________________________________    
    case('PrepareStim')%none to prepare
        ...
        % Randomly select modality (% 0 = LED, 1 = Speaker, 2 = Both  )
        gf.modality = randperm(3,1) - 1;    % Presentation modality
        fprintf('%d\n', gf.modality)
        
        % Determine dominant modality
        if gf.modality == 2 % Audiovisual
            
            % Random selection for audiovisual trials
            gf.domMod = gf.tuitionModality;     % One of unimodal possibilities - force to be visual
            
            % Set cue values
            if gf.domMod == 1
               DA.SetTargetVal( sprintf('%s.LED_cue', gf.stimDevice),  0);
               DA.SetTargetVal( sprintf('%s.Spkr_cue', gf.stimDevice), gf.Spkr_Cue_MF);
            else
               DA.SetTargetVal( sprintf('%s.LED_cue', gf.stimDevice),  gf.LED_Cue_V);
               DA.SetTargetVal( sprintf('%s.Spkr_cue', gf.stimDevice), 0);
            end
            
        else % Unimodal stimuli
            
            gf.domMod = gf.modality;                           
            DA.SetTargetVal( sprintf('%s.LED_cue', gf.stimDevice),  0);
            DA.SetTargetVal( sprintf('%s.Spkr_cue', gf.stimDevice), 0);
        end                
        ...
        % Generate stimulus grid
        gf.stimGrid = generateStimulusGrid(gf.modality, gf.domMod);                             
        gf.LED      = gf.stimGrid(3);
        gf.Speaker  = gf.stimGrid(4);
        gf.LEDmux   = getMUXfamily( gf.LED); 
        gf.SPKmux   = getMUXfamily( gf.Speaker); 
                   
        % Define target spout based on dominant modality
        gf.targetSpout = gf.stimGrid(1,3+gf.domMod);
        ...
        DA.SetTargetVal( sprintf('%s.cueEnable', gf.stimDevice),    gf.modality==2);        % Enable cue presentation

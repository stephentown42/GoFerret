...
%__________________________________________________________________________    
    case('PrepareStim')     
        ...
        for atten_idx = 1 : attn_n
            ...
            attns_unadjusted = repmat( attn_range(atten_idx), size(chans{atten_idx})); 
        ...
        % include rewards)
        T.Interval(end)  = 2 * gf.nSpeakers * median(gf.valveTimes) * gf.fStim;                
        ...
        % Write to buffers        
        if ~DA.WriteTargetVEX( sprintf('%s.intervals', gf.stimDevice), 0, 'F32', T.Interval')
            warning('Failed to write intervals')
        end        
        
        if ~DA.WriteTargetVEX( sprintf('%s.Vs', gf.stimDevice), 0, 'F32', T.Voltage')
            warning('Failed to write intervals')
        end
                
        if ~DA.WriteTargetVEX( sprintf('%s.speakers', gf.stimDevice), 0, 'F32', T.Chan')
...
% Center Response__________________________________________________________        
    case('Play stim')
        ...
        set(h.currentStim,'string',sprintf('%d / %d',currentStim, size(gf.stim,1)))
        
        if currentStim >= size(gf.stim,1)-1,

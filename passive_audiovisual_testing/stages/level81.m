...
%______________________________________    
    case('PrepareStim')
        ...
        sound0         = gf.stim{gf.stimIdx, 1};        % Auditory
        sound1         = gf.stim{gf.stimIdx, 2}+2.5;        % Visual
        gf.soundTime   = length(sound0) ./ gf.fStim;
        
        % Define redundant parameters
        gf.attn     = 43;
        gf.freq     = -1;       
        gf.holdTime = -1 * (length(sound0)/gf.fStim);
        gf.isi = zeros( floor(gf.fStim * 1), 1); 
        
        % Attenuate and envelope
        sound0 = sound0 .* 10^(-(9/20));
%         sound1 = sound1 .* 10^(-(gf.attn/20));
%         sound0 = envelope(sound0, ceil(5e-3*gf.fStim));        
%         sound1 = envelope(sound1, ceil(5e-3*gf.fStim));
%                         
        % Calibrate sounds
%         sound0 = conv(sound0, gf.fltL.flt, 'same');
%         sound1 = conv(sound1, gf.fltR.flt, 'same');
        
        
        % Write sound to buffers
        DA.WriteTargetVEX(sprintf('%s.sound1', gf.stimDevice), 0, 'F32', [sound1; gf.isi]'); % both speakers                
        DA.WriteTargetVEX(sprintf('%s.sound0', gf.stimDevice), 0, 'F32', [sound0; gf.isi]'); % Play from
        
        % Set timing information on TDT
        DA.SetTargetVal( sprintf('%s.stimNPoints',      gf.stimDevice), length(sound0));

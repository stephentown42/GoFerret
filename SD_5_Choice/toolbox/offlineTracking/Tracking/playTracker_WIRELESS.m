function playTracker_WIRELESS(blue, red, video, method)
...
if ~exist('blue','var') || ~exist('red','var')
...
if strcmp(method,'VideoReader')
    ...
    for i = 1 : nI
        ...
        for j = 1 : 1+endIdx(i)-startIdx(i)
            ...
            if all( ~isnan( blue(fCount,2:3))),
                plot( blue(fCount,2), blue(fCount,3), 'xg')
                plot( blue(fRange,2), blue(fRange,3), 'Color',[0.5 0.5 1])
            end
...
if strcmp(method,'mmRead')
    ...
    for i = 1 : nFrames
        ...
        if all( ~isnan( blue(i,2:3))),
            plot( blue(i,2), blue(i,3), 'xg')
            plot( blue(fRange,2), blue(fRange,3), 'Color',[0.5 0.5 1])
        end

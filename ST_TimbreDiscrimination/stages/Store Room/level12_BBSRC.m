...
% Level 12 BBSRC: 
% Probe trials are double vowels with one of two pitch differences. 
% The distractor vowel formants are an additional parameter in the input file
% The pitch values of distractor vowels are also additional 
...
%         gf.side = gf.sides(sideIdx);     
        ...
        if gf.side > 1,
            ...
            % Get distractor pitch
            if gf.side == 2,                % No pitch separation
                        
                gf.dvPitch = gf.pitch;
                
            elseif gf.side == 3,                                            
                
                gf.dvPitch = gf.pitch * 2^(gf.dvF0interval /12);    % N semitones higher                
            end  
            
            % Make distractor
            dtrctr = newMakeVowel2009(  gf.duration/1000,...
                                        gf.fStim, ...
                                        gf.dvPitch,...
                                        gf.dvFormants(1),...
                                        gf.dvFormants(2),...
            ...
            % Calibrate sound
            if ismember(gf.dvFormants,[936,1551,2975,4263],'rows'),
                gf.dvAtten = gf.atten - 5; 
            end
        
            if ismember(gf.dvFormants,[730 2058 2857 4205],'rows'), 
                gf.dvAtten = gf.atten - 2; 
            end
        
                                    
            % envelope to prevent clicking transients
            dtrctr  = envelope(dtrctr,250);
            dtrctr  = dtrctr.*10^(-(gf.dvAtten/20));
            
            % Randomize vowel identity
            gf.formants = eval(sprintf('gf.sound%d',randperm(2,1)-1));
            
            gf.side = gf.side + 10*all(gf.formants==gf.sound1);
        else            
        ...
        % Generate sound        
        vowel  = ComputeTimbreStim(gf.formants);             % create vowel
        isi    = zeros(1, ceil(gf.isi/1000 * gf.fStim));     % add interstimulus interval  
        holdOK = length(vowel) + length(isi);                % point in stimulus that the ferret must hold to        
        vowel  = [vowel+dtrctr, isi, vowel+dtrctr];                        % create two vowels with two intervals
        
        
        % Calculate timing information
        playDelay = gf.holdSamples - holdOK;
        refractS  = playDelay + length(vowel) + ceil(gf.refractoryTime * gf.fStim);
        absentS   = ceil(gf.absentTime * gf.fStim);
        
        % Calibrate sounds
%         sound0 = conv(vowel, gf.fltL.flt, 'same');
%         sound1 = conv(vowel, gf.fltR.flt, 'same');
%         
%         
%         vow = newMakeVowel2009(gf.duration/1000,gf.fStim,gf.pitch,gf.formants(1), ...
%             gf.formants(2),gf.formants(3),gf.formants(4));
%         vow = envelope(vow,gf.fStim*0.025);
%         vow2 =newMakeVowel2009(gf.duration/1000,gf.fStim,gf.dvPitch,gf.dvFormants(1), ...
%             gf.dvFormants(2),gf.dvFormants(3),gf.dvFormants(4));
%         vow2 = envelope(vow2,gf.fStim*0.025);
%         soundsc(vow,gf.fStim)
%         soundsc(vow2,gf.fStim)
%         soundsc(vow+vow2,gf.fStim)
%         
%         soundsc(vowel, gf.fStim)
%         soundsc(sound1, gf.fStim)
        
        sound0 = vowel; %calibrateSignal(sound, gf.calibDir, 1);
        sound1 = vowel; %calibrateSignal(sound, gf.calibDir, 1);
        ...
        DA.SetTargetVal( sprintf('%s.stimNPoints',      gf.stimDevice), length(vowel));        
...
%             gf.responseTime = DA.GetTargetVal(sprintf('%s.lickTime',gf.stimDevice)) ./ gf.fStim;  %Open Ex
            ...
            if rightLick > 0,
                ...
                if gf.side == 1 || gf.side > 10, % Correct response
            ...
            elseif leftLick > 0 
                ...
                if gf.side == 0 || gf.side == 2 || gf.side == 3,

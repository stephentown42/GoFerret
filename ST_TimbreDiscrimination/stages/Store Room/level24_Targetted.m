...
%         if ismember(gf.formants,[730 2058 2857 4205],'rows'), gf.atten = gf.atten - 2; end
        ...
        noise  = rand(1, ceil(0.025 * gf.fStim));         % 100 ms noise
        ...
        if gf.mask > 0,
            ...
            % Add to sound
            sound0(lag:lag+numel(noise)) = sound0(lag:lag+numel(noise)) + [noise 0];
            sound1(lag:lag+numel(noise)) = sound1(lag:lag+numel(noise)) + [noise 0];
        end  
